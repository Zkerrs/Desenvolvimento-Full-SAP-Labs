@AbapCatalog.sqlViewName: 'ZIGLABLFLOWSTD'
...
define view ZI_GLAccountBalanceFlow_Std
  as select from    I_GLAccountLineItem          as Item

    association [0..1] to I_GLAccountText              as GLText      on  Item.ChartOfAccounts = GLText.ChartOfAccounts
                                                                      and Item.GLAccount       = GLText.GLAccount
                                                                      and GLText.Language      = $session.system_language

    association [0..1] to I_GLAccountInChartOfAccounts as MestreConta on  Item.ChartOfAccounts = MestreConta.ChartOfAccounts
                                                                      and Item.GLAccount       = MestreConta.GLAccount

    association [0..1] to I_GLAccountGroupText         as TextoGrupo  on  MestreConta.ChartOfAccounts = TextoGrupo.ChartOfAccounts
                                                                      and MestreConta.GLAccountGroup  = TextoGrupo.GLAccountGroup
                                                                      and TextoGrupo.Language         = $session.system_language

    association [0..1] to I_GLAccountTypeText          as TextoTipo   on  MestreConta.GLAccountType = TextoTipo.GLAccountType
                                                                      and TextoTipo.Language        = $session.system_language

    association [0..1] to I_CostCenterText             as CostCenterT on  Item.ControllingArea        =  CostCenterT.ControllingArea
    ...
    association [0..1] to I_ProfitCenterText           as ProfitCtrT  on  Item.ControllingArea        =  ProfitCtrT.ControllingArea
    ...
    association [0..1] to I_Customer                   as Customer    on  Item.Customer = Customer.Customer

    association [0..1] to I_Supplier                   as Supplier    on  Item.Supplier = Supplier.Supplier

    association [0..1] to I_FixedAsset                 as Asset       on  Item.CompanyCode      = Asset.CompanyCode
                                                                      and Item.MasterFixedAsset = Asset.MasterFixedAsset
                                                                      and Item.FixedAsset       = Asset.FixedAsset

    association [0..1] to I_ProductText                as MaterialT   on  Item.Product       = MaterialT.Product
                                                                      and MaterialT.Language = $session.system_language

    association [0..1] to I_Plant                      as PlantT      on  Item.Plant = PlantT.Plant

    association [0..1] to I_WBSElement                 as WBS         on  Item.WBSElementInternalID = WBS.WBSElementInternalID
